...
public class SberDutyFreeFeed implements Processing {
    ...
    public SberDutyFreeFeed(FtpTransporterImpl ftp, @Qualifier(value = "excelParserDutyFree") ExcelParser excelParser, XmlTransporterImpl xmlTransporter, XmlFinalCreator xmlFinalCreator) {
        this.ftp = ftp;
...
//        // 3. Скачать XLS-фид с FTP
        ...
        if (isXlsSourceCopied) {
            try {
                perfumeStockMap = excelParser.getWarehouseStock(xlsLimitedFile);
